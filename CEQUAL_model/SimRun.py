...
#This only works for two dates within the same calender year.
#If you need to run a simulation for two different calender years, you need to 
#modify the code to handle the different base dates for each year.

#Worthwhile setting up Water_Year_Analogs?
...
# The base date for the Julian Day Calculation shouldn't be hard-coded.
# NOTE: Replace timestamp input with variable set above.
   ...
   # lines.append(f"\n{JDAY_end : >8}{0.0 : >8}\n")
    ...
    try:
        ...
        stable_period = 5  # Time in seconds with no file changes to consider complete
